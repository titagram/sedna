"""Re-learn the 27 writeups against an EXISTING KB root (no rmtree) using the
agent-built bundle as the extract result via a stub host. Sedna's real pipeline
then transitions each quarantined/excluded manifest to verified (or back to
quarantine if the critic rejects)."""
...
def main():
    ...
    assert kb_root.exists(), "root must exist (existing KB) - not destroying it"
    ...
    for m in machines:
        ...
        try:
            ...
            with HadesKnowledgeRuntime.create(host, kb_root, external_source_path=one_src) as rt:
